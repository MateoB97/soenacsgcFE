...
export default ({ Vue }) => {
  ...
  Vue.use(VueAuth, {
    ...
    tokenDefaultName: 'soenacsgc_jwt_token',
  ...
  Vue.axios.defaults.baseURL = 'http://localhost/soenacsgc/back/public/api'
  ...
  Vue.axios.interceptors.response.use(function (response) {
    ...
    if (response.data.message === 'Token has expired and can no longer be refreshed') {
      localStorage.removeItem('soenacsgc_jwt_token')
      // document.location.reload()
    }
    return response
  }, function (error) {
    if (error.response.status === 401) {
      console.log('soenacsgc_jwt_token')
      Vue.auth.refresh()
    }
